...
bot = TeleBot(token="6927628509:AAESBAYd1bkuGQ2OxJ9p3u9Tu9IH55oeYrY",num_threads=45,threaded=True,skip_pending=True,parse_mode='html', disable_web_page_preview=True) # توكن بوت الرشق الاساسي هنا
if not db.get('accounts'):
    db.set('accounts', [])
    pass

admin = 6102414877 #الادمن
if not db.get("admins"):
    db.set('admins', [admin, 5934218120])
...
def start_message(message):
    ...
    if not db.get(f'user_{user_id}'):
        ...
        btn6 = btn('قناة البوت 🩵', url='https://t.me/x_sxxs')
...
def c_rs(call):
    ...
    if data == 'priv_serv':
        ...
        if prem:
            ...
            btn3 = btn('تسليم ارقام',url='https://t.me/xx1axxbot')
            ...
            keys = mk(row_width=2)
            keys.add(btn('ما هي الخدمات الخاصة ؟', url='https://t.me/x_sxxs/55'))
...
- ارسل /start داخل الروبوت ثم قم بتحويل رسالة من الروبوت الي هنا لتحصل علي {join_tmoo_bot} نقطة ❇️'''
    ...
    if data == 'change_point_mill':
        keys = mk(row_width=2)
        btn5 = btn('إضغط هنا لمراسلة المطور ',url='t.me/xYxYxYx')
        btn4 = btn('الذهاب إلى بوت تمويل المليار',url='https://t.me/EEObot?start=5934218120')
        btn6 = btn('‹ رجوع ↻›',callback_data='collect')
        keys.add(btn5)
        keys.add(btn4)
        keys.add(btn6)
        bot.edit_message_text(text='هذا القسم تحت الصيانة اذا كنت تريد تبديل النقاط قم بمراسلة المطور',reply_markup=keys,chat_id=cid,message_id=mid)
...
• لشراء النقاط تواصل مع المطور : @C_N_i '''
